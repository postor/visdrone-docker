...
def proposal_target_layer(rpn_rois, gt_boxes,fg_threshold,stage):
    ...
    rois_per_image = np.inf if cfgs.RCNN_MINIBATCH_SIZE == -1 else cfgs.RCNN_MINIBATCH_SIZE

    fg_rois_per_image = np.round(cfgs.RCNN_POSITIVE_RATE * rois_per_image)
...
def _sample_rois(all_rois, gt_boxes, fg_rois_per_image,
    ...
    bg_inds = np.where((max_overlaps < cfgs.RCNN_IOU_POSITIVE_THR) &
                       (max_overlaps >= cfgs.RCNN_IOU_NEGATIVE_THR))[0]
    # Guard against the case when an image has fewer than fg_rois_per_image
    # foreground RoIs
