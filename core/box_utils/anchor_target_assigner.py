...
import numpy as np
from core.box_utils.cython_utils.cython_bbox import bbox_overlaps
...
if __name__ == '__main__':
    ...
    from core.box_utils import anchor_generator

    test = DatasetLoader()
    imgid, img, gtbox = test.read_next_img(210,1)

    gtbox = np.array(gtbox, np.float32)
    img = np.array(img, np.float32)
    img = np.expand_dims(img, axis=0)
    img_shape = img.shape
    scale = np.array(cfgs.BASE_ANCHOR_SIZE_LIST, dtype=np.float32)
    ratio = np.array(cfgs.ANCHOR_RATIOS, dtype=np.float32)
    f_h = np.array(50, dtype=np.float32)
    f_w = np.array(50, dtype=np.float32)
    anchors = anchor_generator.make_anchors(scale, ratio, f_h, f_w, 16)
    with tf.Session() as sess:
        anchors=sess.run(anchors)

    rpn_labels, rpn_bbox_targets = anchor_target_layer(gtbox, img_shape, anchors)
    print(rpn_labels)
